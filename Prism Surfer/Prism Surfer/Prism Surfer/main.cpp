...
#define FRONT_SPEED 0.1
#define SIDE_SPEED 0.5
...
float map_v, map_c, player_position = 0, ob_time, map_angle = 0;
...
int game_update() {
	...
	while (it != obstacles.end()) {
		if (it.postiion < cam.eye.z) {
			//todo:�浹üũ
			if (it.wall_num * width >= player_position || (it.wall_num + 1) * width <= player_position) {
				return -1;
			}
